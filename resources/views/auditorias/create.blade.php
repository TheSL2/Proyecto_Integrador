                    ...
                    <form action="{{ route('auditorias.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="objetivo" id="objetivo" rows="3" required
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('objetivo') }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="alcance" id="alcance" rows="3" required
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('alcance') }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Área(s) a Evaluar *</label>
                                ...
                                @foreach($areas as $area)
                                    <label class="inline-flex items-center space-x-2">
                                        <input type="checkbox" name="areas[]" value="{{ $area->id }}"
                                               {{ is_array(old('areas')) && in_array($area->id, old('areas')) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <select name="auditor_lider_id" id="auditor_lider_id" required
                                ...
                                @foreach($auditores as $auditor)
                                    <option value="{{ $auditor->id }}" {{ old('auditor_lider_id') == $auditor->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="estado" id="estado" required
                                ...
                                <option value="Planificada" {{ old('estado', 'Planificada') == 'Planificada' ? 'selected' : '' }}>Planificada</option>
                            </select>
                        </div>
